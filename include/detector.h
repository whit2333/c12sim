...
/// - pos:  Position relative to the mother volume, as G4Threestd::vector
...
/// - dimensions:  std::vector of dimensions. Size, units depends on solid type
/// - material:  Volume Material name.
/// - magfield:  Magnetic Field. The std::string "no" means that the field is inherited from the mother volume.
/// - ncopy:  copy number
...
/// - identity:  std::vector of identifiers. Example: superlayer manual 1 type manual 2 segment manual 3 strip manual 4
...
/// A map<std::string, detector> is built by the detector factory. \n
///
...
class detector
{
	...
	public:
		...
		std::string                 name;   ///< Name of the volume. Since this is the key of the STL map, it has to be unique.
		std::string               mother;   ///< Mother Volume name.
		std::string          description;   ///< Volume Description for documentation.
		
		G4ThreeVector           pos;   ///< Position relative to the mother volume, as G4Threestd::vector
		...
		std::vector<double>   dimensions;   ///< std::vector of dimensions. Size, units depends on solid type
		
		std::string             material;   ///< Volume Material name.
		std::string             magfield;   ///< Magnetic Field. The std::string "no" means that the field is inherited from the mother volume.
		...
		std::string          sensitivity;   ///< Defines the Sensitive Detector. possible choices: "no" "hits collection name"
		std::string              hitType;   ///< Hit Process routine name. A Hit Process HitProcess derived class must exists with this name.
		//std::vector<identifier> identity;   ///< std::vector of identifiers. Example: superlayer manual 1 type manual 2 segment manual 3 strip manual 4
		
		int                 scanned;   ///< for use during construction
		
		std::string               system;   ///< detector system
		std::string              factory;   ///< factory that generated the detector
		std::string            variation;   ///< variation that generated the detector
	...
	public:
		...
		G4VPhysicalVolume*         PhysicalV;   ///< Physical Volume
                ...
                G4VSolid * wire_hex_solid;
                G4LogicalVolume * wire_hex_log;
		
	public:
                void print();
		int create_solid(goptions, map<std::string, detector>*);                             ///< Creates the Solid. If it's a Copy Placement, retrieve and assigns LogicV
		int create_logical_volume(map<std::string, G4Material*>*, goptions);                 ///< Creates the Logical Volume.
		...
		friend std::ostream &operator<<(std::ostream &stream, detector);                           ///< Overloaded "<<" for detector class. Dumps infos on screen.

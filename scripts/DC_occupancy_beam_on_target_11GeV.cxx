...
int DC_occupancy_beam_on_target_11GeV(
      int start_run = 230,
      int end_run   = 289, 
      int n_gen     = 310000,
      int n_proc    = 0,
...
{
   // n_gen  : the number of incident electrons per time window defining the occupancy
   //         at a fixed luminosity
   // n_proc : number of events to process, if it is not set all events  are processed

   double norm     = 1;
   double norm_sim = 1.0; // numerator weight
   if(aNumber == 0 ) aNumber = start_run;

   TChain * t =  new TChain("clasdigi_hits");
   for(int i = start_run; i<=end_run; i++) {
      t->Add(Form("data/rootfiles_beam_on_target_11GeV/clas12sim%d.root",i));
   } 

   int N_events_total = t->GetEntries();
   if(n_proc == 0 ) n_proc = N_events_total;
   norm = double(n_proc)/double(n_gen);

   if( N_events_total <= 0  ) {
      std::cout << " No events in chain!" << std::endl;
      return -234;
   }

   // Defines the occupancy time windows, is essentially DC region's maximum drift time
   ...
   for(int iEvent =  0 ; iEvent < nEvents & iEvent < n_proc; iEvent++) {
      ...
      for(int i = 0; i < event->fDCEvent.fNParticleHits; i++) {
         ...
         if( ( ahit->fGlobalPosition.T() < 500.0     ) && // 500 ns
            ...
            if( nWireHits[sec-1][sl-1][lay-1][wire-1] == 1 ){
               ...
               double val  =  h->GetBinContent(bin) + norm_sim/(time_norm*norm);
               h->SetBinContent(bin,val);

               // ----------------------
               // 1D hists
               bin = ahit->fDCWire.fWire;
               val = fOccupancies[sec-1][sl-1]->GetBinContent(bin) + norm_sim/(time_norm*norm*6.0);
               fOccupancies[sec-1][sl-1]->SetBinContent(bin, val);

               bin = ahit->fDCWire.fWire;
               val = fLayerOccupancies[sec-1][sl-1][lay-1]->GetBinContent(bin) + norm_sim/(time_norm*norm);
               fLayerOccupancies[sec-1][sl-1][lay-1]->SetBinContent(bin, val);

               int sec_chan = 6*(sl-1) + (lay-1);
               val = fSLAveraged[sec_chan]->GetBinContent(bin) + norm_sim/(time_norm*norm*6.0);
   ...
   c0->SaveAs(Form("data/results/DC_occupancy/DC_occupancy_beam_on_target_layeravg_%d.png",aNumber));
   c0->SaveAs(Form("data/results/DC_occupancy/DC_occupancy_beam_on_target_layeravg_%d.pdf",aNumber));
   ...
   c0->SaveAs(Form("data/results/DC_occupancy/DC_occupancy_beam_on_target_allsec_yfixed_%d.pdf",aNumber));
   ...
   c0->SaveAs(Form("data/results/DC_occupancy/DC_occupancy_beam_on_target_allsec_yfixed1_%d.pdf",aNumber));
   ...
   c->SaveAs(Form("data/results/DC_occupancy/DC_occupancy_beam_on_target_%d.png",aNumber));
   c->SaveAs(Form("data/results/DC_occupancy/DC_occupancy_beam_on_target_%d.pdf",aNumber));

...
int detector::create_logical_volume(map<string, G4Material*> *MMats, goptions gemcOpt)
{
   ...
   //if( (name.find("layer") != string::npos) )
   //{
   //   if( (mother.find("region") != string::npos) )
   //   {

   //      std::cout << name << " AND " << mother << std::endl;
   ...
   ////polygon.push_back({ -d_layer_sl1, -h_hex} );
   ////polygon.push_back({       -h_hex,    0.0} );
   //////wire_hex_solid = new G4ExtrudedSolid("wire hex solid", polygon,zsections);
   ////G4Box * subtraction_box = new G4Box("wire_hex_solid",    30*cm, 4.0*h_hex, 1.0*h_hex);
   ////G4Box * subtraction_box2 = new G4Box("wire_hex_solid2",  30*cm, 4.0*h_hex, 1.0*h_hex);
   ////G4Box * subtraction_box3 = new G4Box("bigbox",           30*cm,  9.0*h_hex,    9*h_hex);
   ////   //G4ExtrudedSolid("wire hex solid", polygon, 0.5*cm, G4TwoVector(0.0,0.0),1.0, G4TwoVector(0, 0), 1.0);
   ...
   ////yRot3->rotateX(M_PI/6.*rad);                     // Rotates 30 degrees

   ////G4ThreeVector zTrans(0, 0, 0);

   ////G4VSolid* unionMoved  = new G4IntersectionSolid("BoxCylinderMoved",  subtraction_box,  subtraction_box2, yRot,  zTrans);
   ////G4VSolid* unionMoved2 = new G4IntersectionSolid("BoxCylinderMoved2", unionMoved,       subtraction_box2, yRot2, zTrans);
   ////G4VSolid* unionMoved3 = new G4IntersectionSolid("BoxCylinderMoved3", subtraction_box3, unionMoved2,      yRot3, zTrans);
   ////wire_hex_solid = unionMoved3;

   ////      //if(!wire_hex_log) {

   ////       G4VSolid * region_1_solid = new G4GenericTrap("RItrap",10*cm, clas12::geo::RegionTrapPoints(1));

   //// wire_hex_log= new G4LogicalVolume(region_1_solid, (*MMats)[material], "wire_hex_log");
   ////       G4VPhysicalVolume* wire_hex_phys =
   ////          new G4PVPlacement(G4Transform3D::Identity, // translation position
   ////             wire_hex_log,              // its logical volume
   ////             "wire_hex_phys",               // its name
   ////             GetLogical(),                // its mother (logical) volume
   ////             false,                   // no boolean operations
   ////             0);                      // its copy number 
   ////new G4PVReplica( "wire_hex_replica",
   ////      wire_hex_log,
   ////      LogicV, // OR G4VPhysicalVolume*
   ////      kYAxis,
   ////      112,
   ////      h_hex,
   ////      0 );
   ...
   //      //}
   //      //detect.SolidV = wire_hex_solid;
   //      //detect.GetLogical()->SetSolid(wire_hex_solid);


   //      //G4double maxStep = 0.1*mm;
   //      //GetLogical()->SetUserLimits(new G4UserLimits(maxStep));
   //      //wire_hex_log->SetUserLimits(new G4UserLimits(maxStep));
...
void detector::print(){
   ...
   std::cout << "   magfield : " << magfield << "\n";
   std::cout << "      ncopy : " << ncopy << "\n";
   std::cout << "      pMany : " << pMany << "\n";

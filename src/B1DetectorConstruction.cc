...
{  
   ...
   G4double minEps = 1.0e-3;//1.0e-6;  //   Minimum & value for smallest steps
   G4double maxEps = 1.0e-2;//1.0e-5;  //   Maximum & value for largest steps

   fieldMgr->GetChordFinder()->SetDeltaChord(0.01);//1.0e-5);
   fieldMgr->SetMinimumEpsilonStep( minEps );
   fieldMgr->SetMaximumEpsilonStep( maxEps );
   fieldMgr->SetDeltaOneStep( 0.01);//1.0e-6 * mm );  // 0.5 micrometer
